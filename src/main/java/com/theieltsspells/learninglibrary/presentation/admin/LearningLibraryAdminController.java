...
public class LearningLibraryAdminController {
    ...
    @GetMapping("/resources/{id}") public LearningResourceResponse resource(@PathVariable UUID id) { return service.getResource(id); }
    @GetMapping("/resources/{id}/files") public List<LearningResourceFileResponse> resourceFiles(@PathVariable UUID id) { return service.listResourceFiles(id); }
    @PostMapping("/resources") public ResponseEntity<LearningResourceResponse> createResource(
            @Valid @RequestBody LearningResourceRequest request, @AuthenticationPrincipal Jwt jwt) {
        var result = service.createResource(request, UUID.fromString(jwt.getSubject()));
        return ResponseEntity.created(URI.create("/api/v1/admin/library/resources/" + result.id())).body(result);
    }
    @PutMapping("/resources/{id}") public LearningResourceResponse updateResource(@PathVariable UUID id, @Valid @RequestBody LearningResourceRequest request) { return service.updateResource(id, request); }
    @DeleteMapping("/resources/{id}") public ResponseEntity<Void> archiveResource(@PathVariable UUID id) { service.archiveResource(id); return ResponseEntity.noContent().build(); }
    ...
    public ResponseEntity<LearningResourceFileResponse> uploadResourceFile(@PathVariable UUID id,
        ...
        var result = service.uploadResourceFile(id, fileRole, file, UUID.fromString(jwt.getSubject()));
    ...
    public ResponseEntity<Void> deleteResourceFile(@PathVariable UUID resourceId, @PathVariable UUID fileId) {
        service.deleteResourceFile(resourceId, fileId); return ResponseEntity.noContent().build();
    }
    ...
    public ResponseEntity<MediaAssetResponse> uploadMedia(@RequestParam MultipartFile file,
            @AuthenticationPrincipal Jwt jwt) {
        var result = service.uploadMedia(file, UUID.fromString(jwt.getSubject()));
        return ResponseEntity.created(URI.create("/api/v1/admin/library/media/" + result.id())).body(result);
    }
    @DeleteMapping("/media/{fileId}")
    public ResponseEntity<Void> deleteMedia(@PathVariable UUID fileId) {
        service.deleteMedia(fileId); return ResponseEntity.noContent().build();
    }
    ...
    @GetMapping("/exercises/{id}") public ExerciseTemplateResponse exercise(@PathVariable UUID id) { return service.getExercise(id); }
    @PostMapping("/exercises") public ResponseEntity<ExerciseTemplateResponse> createExercise(
            @Valid @RequestBody ExerciseTemplateRequest request, @AuthenticationPrincipal Jwt jwt) {
        var result = service.createExercise(request, UUID.fromString(jwt.getSubject()));
        return ResponseEntity.created(URI.create("/api/v1/admin/library/exercises/" + result.id())).body(result);
    }
    @PutMapping("/exercises/{id}") public ExerciseTemplateResponse updateExercise(@PathVariable UUID id, @Valid @RequestBody ExerciseTemplateRequest request) { return service.updateExercise(id, request); }
    @DeleteMapping("/exercises/{id}") public ResponseEntity<Void> archiveExercise(@PathVariable UUID id) { service.archiveExercise(id); return ResponseEntity.noContent().build(); }
}
