...
public class StudentAdminController {
    ...
    public PageResponse<StudentSearchResponse> search(@RequestParam(defaultValue = "") String q,
            @PageableDefault(size = 20, sort = "studentCode") Pageable pageable) {
        return PageResponse.from(service.search(q, pageable));
    }

    @GetMapping("/{id}")
    @Operation(summary = "Xem hồ sơ chi tiết học viên")
    public StudentDetailResponse get(@PathVariable UUID id) { return service.get(id); }
}
